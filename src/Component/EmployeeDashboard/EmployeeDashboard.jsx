...
const EmployeeDashboard = () => {
    ...
    const [salaryUnlocked, setSalaryUnlocked] = useState(
        typeof window !== 'undefined'
            ? sessionStorage.getItem(SALARY_UNLOCK_KEY) === '1'
            : false
    );
        ...
        const load = async () => {
            try {
                const [ci, co, otS, otE] = await Promise.all([
                    axiosProtect.get('/getCheckInInfo', {
                        params: { userEmail: user.email, date: todayStr },
                    }),
                    axiosProtect.get('/getCheckOutInfo', {
                        params: { userEmail: user.email, date: todayStr },
                    }),
                    axiosProtect.get('/getStartOTInfo', {
                        params: { userEmail: user.email, date: todayStr },
                    }),
                    axiosProtect.get('/getStopOTTime', {
                        params: { userEmail: user.email, date: todayStr },
                    }),
                ]);
    ...
    const thisMonthAttendance = useMemo(() => {
        return (attendanceInfo || []).filter(
            (a) => a?.email === user?.email && a?.month === monthName
        );
    }, [attendanceInfo, user?.email, monthName]);

    const presentCount = thisMonthAttendance.length || 0;
    const lateCount =
        thisMonthAttendance.filter((a) => !!a.lateCheckIn).length || 0;
    const totalOTSecondsThisMonth = thisMonthAttendance.reduce(
        (acc, a) => acc + (Number(a.totalOTInSeconds) || 0),
        0
    );
    ...
    const handleCheckIn = async () => {
        try {
            const payload = {
                date: moment().format('YYYY-MM-DD'), // FIXED
                month: moment().format('MMMM'),
                checkInTime: Date.now(),
                displayTime: moment().format('hh:mm:ss A'),
                signInTime: moment(user?.metadata?.lastSignInTime).format(
                    'DD MMM hh:mm:ss A'
                ),
                email: user.email,
            };
    ...
    const handleCheckOut = async () => {
        try {
            ...
            const morningShiftEnd = now
                .clone()
                .startOf('day')
                .add(14, 'hours')
                .valueOf(); // 2:00 PM
            const generalShiftEnd = now
                .clone()
                .startOf('day')
                .add(18, 'hours')
                .valueOf(); // 6:00 PM
            const eveningShiftEnd = now
                .clone()
                .startOf('day')
                .add(22, 'hours')
                .valueOf(); // 10:00 PM
            ...
            const isEarly =
                (myShiftName === 'Morning' &&
                    now.valueOf() < morningShiftEnd) ||
                (myShiftName === 'General' &&
                    now.valueOf() < generalShiftEnd) ||
            ...
            dispatch(setRefetch(!refetch));
            res.data?.message === 'Check-out successful'
                ? toast.success(res.data.message)
                : toast.warning(
                      res.data?.message || 'Check-out response unknown'
                  );
        } catch {
    ...
    const handleStartOT = async () => {
        try {
            ...
            const res = await axiosSecure.post(
                '/employee/startOverTime',
                payload
            );
            dispatch(setRefetch(!refetch));
            res.data?.message === 'Over time started'
                ? toast.success(res.data.message)
                : toast.warning(res.data?.message);
        } catch {
    ...
    const handleStopOT = async () => {
        try {
            ...
            const res = await axiosSecure.post(
                '/employee/stopOverTime',
                payload
            );
            dispatch(setRefetch(!refetch));
            res.data?.message === 'OT stop successful'
                ? toast.success(res.data.message)
                : toast.warning(res.data?.message);
        } catch {
    ...
    const maskedSalary = '••••';
    const monthlySalary = Number(salaryAndPF?.salary || 0).toLocaleString(
        undefined,
        { minimumFractionDigits: 2, maximumFractionDigits: 2 }
    );
    const monthlyPF = Number(
        (Number(salaryAndPF?.salary || 0) * 5) / 100
    ).toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
    });
    const pfBalance = Number(salaryAndPF?.pfContribution || 0).toLocaleString(
        undefined,
        { minimumFractionDigits: 2, maximumFractionDigits: 2 }
    );
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                    <div className="flex items-center gap-4">
                        <img
                            src={
                                employee?.photo ||
                                'https://i.ibb.co/6WfW6mS/user.png'
                            }
                        ...
                        <div>
                            ...
                            <div className="mt-2 text-sm text-gray-600 flex flex-wrap gap-4">
                                ...
                                {employee?.address && (
                                    <span className="inline-flex items-center gap-1">
                                        <MapPin size={14} /> {employee.address}
                                    </span>
                                )}
                                {employee?.DOB && (
                                    <span className="inline-flex items-center gap-1">
                                        <CalendarDays size={14} /> DOB:{' '}
                                        {employee.DOB}
                                    </span>
                                )}
                            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white rounded-xl shadow border border-gray-100 p-6">
                    ...
                    <div className="mt-3 p-3 bg-blue-50 text-sm rounded-lg border border-blue-100 flex items-start">
                        ...
                        <div>
                            Your salary is hidden by default on this device.
                            Click <b>Unlock Salary</b> and enter your PIN to
                            view it. It will re-lock when you close this tab.
                        </div>
                    </div>
                </div>
                                    ...
                                    >
                                        ...
                                        <td>
                                            <div className="flex items-center gap-1">
                                                ...
                                                {row.lateCheckIn && (
                                                    <sup className="ml-1 bg-red-500 px-2 py-0.5 rounded-2xl text-white text-[10px]">
                                                        late
                                                    </sup>
                                                )}
                                            </div>
            ...
            <div className="bg-white rounded-xl shadow border border-gray-100 p-6">
                ...
                <div className="mt-2 p-3 bg-blue-50 text-sm rounded-lg border border-blue-100 flex items-start">
                    ...
                    <div>
                        ...
                        Never share your salary PIN. Your salary view unlock
                        lasts only while this tab is open.
                    </div>
                </div>
            </div>
